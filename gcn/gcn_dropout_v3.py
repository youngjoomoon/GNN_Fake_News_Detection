...
keyword_list_freq = keyword_df.value_counts().head(100).reset_index()['keyword'].to_list()
...
keyword_embeddings = []
for idx, keywords in df.iterrows():
    key_embeddings = torch.zeros(1, 100)
    for keyword in keywords['keybert_keywords'].split(','):
...
model = GCN(g.ndata['features'].shape[1], 64, 2, 0.4)
...
torch.save(model.state_dict(), 'gcn_model_v4')
